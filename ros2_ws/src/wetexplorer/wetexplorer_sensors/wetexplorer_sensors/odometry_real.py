...
# JUST MODIFY THE SECTION MARKED "ALGORITHM BLOCK" (COVARIANCES, ETC.)
...
class ForwardKinematicsNode(Node):
    def __init__(self):
        ...
        # Topics
        self.declare_parameter("input_topic", "/robo/sys")
        ...
        self.get_logger().info("ForwardKinematicsNode initialized")

    def velocity_callback(self, msg: String):
        # Parse comma-separated string from /robo/sys
        tokens = msg.data.split(",")
        if len(tokens) < 5:
            self.get_logger().warn("Not enough tokens in velocity message")
            return

        try:
            # These are in RPM (The string is defined by the robot chassis drivers)
            omega_L_rpm = float(tokens[4])
            omega_R_rpm = float(tokens[3])
        except ValueError as e:
            self.get_logger().error(f"Failed to parse RPM values: {e}")
            return
        ...
        # Convert RPM -> rad/s (these are the wheel angular velocities in rad/s)
        omega_L = omega_L_rpm * 2.0 * math.pi / 60.0
        omega_R = omega_R_rpm * 2.0 * math.pi / 60.0


        
        # Velocities in World Coordinates
        vx = 0.0
        vy = 0.0
        omega = 0.0
        ...
        #   - Pose in world frame: self.x, self.y, self.theta
        #   - Time step: dt
        #
        # Define covariances for:
        #   Pose:  cov_x, cov_y, cov_theta
        #   Twist: cov_vx, cov_vy, cov_omega
        #
        # By default everything is zero (no uncertainty).
        # ----------------------------------------------------
